...
def extract_verified_claims(retrieved_chunks: List[Dict[str, Any]]) -> ClaimsResponse:
    ...
    using the modern Google GenAI SDK (gemini-2.0-flash).
    """
    import os
    if not os.environ.get("GEMINI_API_KEY"):
        # Fallback to realistic mock claims when API key is missing
    ...
    for chunk in retrieved_chunks:
        ...
        text = chunk.get("text", "")
        # Handle cases where page is in metadata or top-level dictionary
        page = chunk.get("page")
